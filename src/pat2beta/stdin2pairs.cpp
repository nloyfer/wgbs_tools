...
int Pat2Beta::proc_line(std::vector<std::string> tokens) {
    ...
    if (( site + read_len - 1 < start ) || (site >= end)){ return 0; }

    if (read_len < 2) { return 0;}

    for(int i = 1; i < read_len; i++ ) {
        int cur_char = pattern[i];
        int pre_char = pattern[i - 1];
        int cur_ind = site - start + i;
        if ((cur_ind >= nr_sites) || (cur_ind < 0)) { continue; }
        if      ((pre_char == 'T') && (cur_char == 'T')) { tt[cur_ind] += count; }
        else if ((pre_char == 'T') && (cur_char == 'C')) { tc[cur_ind] += count; }
        else if ((pre_char == 'C') && (cur_char == 'T')) { ct[cur_ind] += count; }
        else if ((pre_char == 'C') && (cur_char == 'C')) { cc[cur_ind] += count; }
    }
